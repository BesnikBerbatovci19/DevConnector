...
router.get('/', (req, res) =>{
        ...
        .catch(err => res.status(404).json({nopostfounds: 'No posts '}));
})
    ...
    .then(profile => {
        ...
        .then(post =>{
            ...
            if(post.user.toString() !== req.user.id){
                   return res.status(401).json({noautherized: 'Not Autherizated'}); 
            }
    ...
    .then(profile => {
        ...
        .then(post =>{
           if(post.likes.filter(like => like.user.toString() === req.user.id).length > 0){
                return res.status(400).json({ alreadylike: 'User alrady liked this post'})
           }
...
, passport.authenticate('jwt', { session: false}), (req, res) =>{
    ...
    .then(profile => {
        ...
        .then(post =>{
           if(post.likes.filter(like => like.user.toString() === req.user.id).length === 0){
                return res.status(400).json({ notlike: 'You hav not yet like'})
           }
           // Get remove index
...
router.delete('/comments/:id/:comment_id', passport.authenticate('jwt', {session: false}), (req, res)=>{
    ...
    Post.findById(req.params.id)
    .then(post => {
      // Check to se if comment exists

      if(post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length === 0){
          return res.status(404).json({comentnotexsist: 'comment not exsist'})
      }
